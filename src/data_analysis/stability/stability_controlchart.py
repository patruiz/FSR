...
def stability_analyze(file_path, img_dir):
        ...
        plt.savefig(img_dir, bbox_inches = 'tight', dpi = 100)
        ...
        print(f"Average: {round(np.average(ohms))}")
        print(f"Standard Dev: {round(np.std(ohms))}")
        print(f"Max: {round(np.max(ohms))}")
        print(f"Min: {round(np.min(ohms))}")
        print(f"Range: {round(np.max(ohms) - np.min(ohms))}")
        print(f"Upper Control Limit (UCL): {round(ucl)}")
        print(f"Lower Control Limit (LCL): {round(lcl)}")
...
for i in names:
    ...
    file_path = os.path.join(os.getcwd(), 'data', FSR_dir, 'stability', full_filename)
    full_imgname = file_name + ".jpg"
    img_dir = os.path.join(os.getcwd(), 'data', FSR_dir, 'images', 'WeightIntervals', full_imgname)
